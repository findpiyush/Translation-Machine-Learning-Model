...
with open(eng_data_path, 'r', encoding='utf-8') as file:
    eng_lines=file.read().split("\n")
with open(hin_data_path, 'r', encoding='utf-8') as file:
    hin_lines=file.read().split("\n")

for line in eng_lines[: min(num_samples, len(eng_lines)-1)]:
    input_text= line
...
for line in hin_lines[: min(num_samples, len(hin_lines)-1)]:
    target_text= "\t"+line
